...
router.post("/poems", (req, res) => {
  try {
    ...
    if (title.length > 500) {
      return res
        .status(400)
        .json({ error: "Title too long (max 500 characters)" });
    }

    if (content.length > 10000) {
      return res
        .status(400)
        .json({ error: "Content too long (max 10000 characters)" });
    }
...
router.put("/poems/:id", (req, res) => {
  try {
    ...
    const poems = readPoems();
    const index = poems.findIndex((poem) => poem._id === id);
    ...
    if (writePoems(poems)) {
      console.log("✅ Poem updated successfully:", id);
      res.setHeader("Content-Type", "application/json; charset=utf-8");
      res.json(poems[index]);
    } else {
      res.status(500).json({ error: "Failed to save updated poem" });
    }
  } catch (error) {
